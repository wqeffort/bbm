...
<div class="join_cont">
    <div>请您输入需要充值的用户手机号码: </br><input style="text-align: center;" type="number" maxlength="11" name="phone"></div>
    <div style="background: #fffbe1;">请输入需要充值的积分: <input type="number" style="text-align: center;" placeholder="请输入1000的整数倍" name="point"></div>
...
function select() {
    ...
    if(!isPhone(phone)) {
        ...
        //提示
        layer.open({
            content: '请正确输入1000的整数倍充值积分'
            ,skin: 'msg'
            ,time: 2 //2秒后自动关闭
        });
    }else{
        load();
        $.post('{{ url('join/recharge/select') }}/'+phone+'/'+point+'',{
            "_token" : '{{ csrf_token() }}'
        },function (ret) {
