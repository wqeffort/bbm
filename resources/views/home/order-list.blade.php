        ...
        @foreach ($order as $element)
            <div class="orderBox">
                <dl>
                    ...
                    <dd>
                        <ul class="payInfo">
                            <li>应付积分: <b>{{ $element->totalPoint }}</b> 元</li>
                            {{-- <li>实付积分: <b>{{ intval($element->totalPoint  * $element->sale) }}</b> 积分</li> --}}
                        </ul>
                    </dd>
                    <dd style="margin-top: .5rem;">@if ($element->status == '0')
                        <a style="background: #C40000;" href="{{ url('pay') }}/{{ $element->orderNum }}">现在去支付 >></a>
                        <a href="javascript:;" onclick=del("{{ $element->orderNum }}")>删除订单</a>
                    @else
                        @if ($element->express == 2)
                            <a href="javascript:;" onclick=getExpressInfo("{{ $element->orderNum }}")>点击查看物流信息 >></a>
                        @else
                            <a href="javascript:;" onclick="goBtn()">催促发货 >></a>
                        @endif
                        
                    @endif</dd>
                    @if ($element->status == '0')
                         <span style="background: green;" class="orderBtn">未支付</span>
                    @else
                         <span class="orderBtn">已经支付</span>
                    @endif
                </dl>
            </div>
        @endforeach
