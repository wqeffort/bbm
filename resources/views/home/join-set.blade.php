...
<ul>
    <li onclick="setNickname()">
        <p>用户姓名:</p>
        <span>{{ $info->user_name }} <i class="fa fa-angle-right"></i></span>
    </li>
    <li>
        <p>用户头像</p>
        <span><img style="width: 2.5rem;border-radius: 50%;" src="{{ asset($info->user_pic) }}"><i class="fa fa-angle-right"></i></span>
    </li>
    <li onclick="setUid()">
        <p>协议类型</p>
        <span>
    ...
    <a href="{{ url('join/out') }}">
        <li style="text-align: center;display: flow-root;">
        <p>退出登录</p>
    </li>
    </a>
...
<script type="text/javascript">
    ...
    function setCashPassword() {
        location.href = '{{ url('join/set/setPassword') }}';
    }
    ...
    function sub() {
        ...
        if (password == "" || password_again == "") {
            ...
            load();
            $.post('{{ url('user/set/setPasswordUpdate') }}', {
                "_token" : '{{ csrf_token() }}',
                "password" : password,
                "code" : code
            }, function(ret) {
