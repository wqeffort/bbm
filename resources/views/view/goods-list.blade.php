            ...
            @foreach ($goods as $element)
                <a href="{{ url('view/goods') }}/{{ $element->id }}">
                    <ul style=" display: flex;justify-content: left;overflow: hidden;border-bottom: 1px solid #EEE;padding: .5rem 0;color: #333;">
                        <li><img style="width: 8rem;height:8rem;" src="{{ url($element->goods_pic) }}" alt="{{ $element->goods_name }}"></li>
...
$(window).scroll(function() {
    ...
    if (h_scroll == (h_document - h_window)) {
        start == 2
        // 获取新的数据
            $.post('{{ url('view/goods/list/brand') }}/{{ $data }}/'+page,{
                "_token" : '{{ csrf_token() }}'
            },function (ret) {
                var obj = $.parseJSON(ret);
                // console.log(obj);
