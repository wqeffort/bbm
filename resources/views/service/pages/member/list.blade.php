				...
				<tbody id="table">
					@foreach ($data as $element)
						<tr data-id="{{ $element->id }}">
							...
							<td class="td-manage">
								<a href="javascript:;" title="编辑" target="tabAdd" _href="{{ url('service/user/info') }}/{{ $element->user_uuid }}" onclick="parent.tab.tabAdd('加盟商信息编辑','{{ url('service/join/info') }}/{{ $element->user_uuid }}','join-info_{{ $element->user_uuid }}')">
									<i class="layui-icon">&#xe642;</i>
								</a>
								<a href="javascript:;" title="充值" onclick="parent.tab.tabAdd('加盟商充值选项','{{ url('service/join/pay') }}/{{ $element->user_uuid }}','join-pay_{{ $element->user_uuid }}')">
									<i class="layui-icon">&#xe65e;</i>
								</a>
							</td>
						</tr>
			...
			$.post('{{ url('service/search/user') }}', {
				...
				if (obj.status == 'success') {
					...
					$.each(obj.data, function(index, val) {
						...
						<td class="td-manage">\
							<a href="javascript:;" title="编辑" target="tabAdd" _href="{{ url('service/user/info') }}/'+val.user_uuid+'" onclick=parent.tab.tabAdd("会员信息编辑","{{ url('service/user/info') }}/'+val.user_uuid+'","user-info_'+val.user_uuid+'")>\
									<i class="layui-icon">&#xe642;</i>\
								</a>\
							<a href="javascript:;" title="充值" target="tabAdd" _href="{{ url('service/user/pay') }}/'+val.user_uuid+'" onclick=parent.tab.tabAdd("会员充值选项","{{ url('service/user/pay') }}/'+val.user_uuid+'","user-pay_'+val.user_uuid+'")>\
								<i class="layui-icon">&#xe65e;</i>\
							</a>\
						</td>\
					</tr>'
