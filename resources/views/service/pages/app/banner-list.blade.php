...
@section('body')
		...
		<div class="weadmin-body">
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 10px;">
			  	<legend>APP Banner Manage</legend>
			</fieldset>
			<div class="weadmin-block">
				<button class="layui-btn" onclick="WeAdminShow('添加轮播','{{ url('service/app/banner/add') }}',600,400)"><i class="layui-icon"></i>添加轮播</button>
				<span class="fr" style="line-height:40px">共有数据：{{ $banner->count() }} 条</span>
			</div>
				...
				<tbody id="table">
					@foreach ($banner as $element)
						<tr data-id="{{ $element->id }}">
							...
							<td class="td-manage">
								<a href="javascript:;"  onclick="WeAdminShow('编辑轮播','{{ url('service/app/banner/edit') }}/{{ $element->id }}',600,400)">
...
<script type="text/javascript">
	function status(id) {
		loading();
		$.post('{{ url('service/app/banner/status') }}/'+id, {
			"_token" : '{{ csrf_token() }}'
		}, function(ret) {
